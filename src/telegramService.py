	...
	def loadUsers(self):
		if self.__bot_id != None and self.__bot_id != "":
			...
			try:
				...
				log.error(f"Erro ao buscar novos usuários: {erro.args}")
		else:
			log.warning("ATENÇÃO!")
			log.warning("O bot de monitoramento não está devidamente configurado!")
			log.warning("Revise suas configurações.")
